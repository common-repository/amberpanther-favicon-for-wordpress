<?php
/* 
Control center for AmberPanther Plugins
*/
...
class ap_plugin_AmbraPanthera {
	...
	function ap_plugin_AmbraPanthera(){
		...
		add_action('wp_head',array(&$this, 'ap_load_jquery_intheme'));
	...
	function ap_admin_head_addition(){	
		//$theurl =  WP_PLUGIN_URL .'/'. str_replace(basename( $this->thefile),"",plugin_basename($this->thefile));
		//$admin_stylesheet_url = $theurl.'options/dashboard/dashboard.css';
		//echo '<link rel="stylesheet" href="'. $admin_stylesheet_url . '" type="text/css" />';
		echo '<style type="text/css">';
		echo '/*<![CDATA[*/';
		echo '#ap_footer{clear:both}';
		echo '#ap_footer.postbox h3,#ap_footer.postbox{-moz-border-radius-topleft:0;-moz-border-radius-topright:0;-moz-border-radius-bottomleft:6px;-moz-border-radius-bottomright:6px;height:25px;}';
		echo '#ap_footer.postbox h3{text-align:center;margin:0;cursor:auto;line-height:25px;font-weight:normal;font-size:12px}';
		echo '#ap_footer img{margin-bottom:-3px;}';
		echo '.ap_indent{padding-left:30px}';
		echo '/*]]>*/';
		echo '</style>';
				...
				<div id="ap_quicklinks" class="postbox ">
					...
					<div class="inside">
						<ul class="ap_listdisc">
							...
							<li><a href="<?php echo $this->homeurl; ?>" target="_blank" title=""><?php echo $this->pluginname; ?> Home</a></li>
							...
							?>
							<li>&nbsp; </li>
							<li><a href="http://chest.amberpanther.com/redir/csstut" target="_blank" title="">CSS Tutorial</a></li>
						</ul>
					</div><!-- end inside-->
				</div><!-- end side-box-1-->
...
?>
	...
	function ap_resetoptions_box(){ ?>
		...
		<div class="handlediv" title="Click to toggle"><br /></div>
		<h3 class="hndle"><span>Reset</span></h3>
		<div class="inside">
		<p>Reset all <?php echo $this->pluginname; ?> options to default values | <strong>Warning: All your customization will be lost!</strong></p>
		<p class="submit" style="float:none"><button class="button" type="button" id="proceed-reset">Proceed</button></p>
		<div id="ap_reset-confirm" style="margin-top:10px;color:#900;display:none">
			Are you sure you want to proceed and reset?
			<p class="submit" style="float:none;margin-top:10px">
			<input class="button" type="submit" value="Reset" name="Reset"/><button class="button" type="button" id="cancel-reset" style="margin-left:20px">Cancel</button>
			</p>
		</div>
...
<?php	}
	...
	function ap_news(){
		...
		if ($maxitems == 0){
			...
			foreach ( $rss_items as $item ){
				$content .= '<li>';
				$content .= '<a href="'.clean_url( $item->get_permalink(), $protocolls=null, 'display' ).'" title="Posted on '.$item->get_date('j F Y | g:i a').'">'. $item->get_title() .'</a> ';
				$content .= '</li>';
			}
			$content .= '</ul>';
		}
	...
	function ap_db_rsswidget(){
		...
		if ($maxitems != 0){
			...
			foreach ( $rss_items as $item ){
				echo '<li>';
				echo '<a href="'.clean_url( $item->get_permalink(), $protocolls=null, 'display' ).'">'. $item->get_title() .'</a> ';
